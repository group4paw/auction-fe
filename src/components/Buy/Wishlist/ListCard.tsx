...
export default function ListCard() {
	...
	const fetchData = async (userId: any) => {
		...
		let wishlist = [] as any[];
		try {
			await axios.get("https://auction-api-4.vercel.app/auction/").then((res) => {
				auctions = res.data;
			});

			await axios.get("https://auction-api-4.vercel.app/wishlist/" + userId).then((res) => {
				wishlist = res.data;
			});
			...
			{!isLoading ? (
				listAuction.length > 0 ? (
					...
					<div className="w-screen flex justify-center items-center h-1/2">
						<p className="text-2xl text-neutral-700">You dont have any wishlist</p>
					</div>
				)
			) : (
				<div className="w-screen flex justify-center items-center h-1/2">
